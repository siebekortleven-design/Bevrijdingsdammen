    ...
    <div className="app">
      ...
      <div className={`board-and-eval ${appMode === 'online' ? 'no-eval-bar' : ''}`}>
        <div className="board-wrapper">
          ...
          <div className="board">{cells.map(({ row, col, isDark, playPos }) => { const piece = playPos ? board[playPos] : null; const isSelected = playPos === selected; const isValidDest = playPos && validDestsSet.has(playPos); return (<div key={`${row}-${col}`} className={`cell ${isDark ? 'dark' : 'light'} ${isSelected ? 'selected' : ''} ${isValidDest ? 'valid-dest' : ''}`} onClick={() => playPos && handleClick(playPos)}>{piece && piece !== EMPTY && (<div className={`piece ${getPieceClass(piece)}`}>{isKingPiece(piece) && <span className="crown">♛</span>}</div>)}{isValidDest && !piece && <div className="move-hint" />}{playPos && <span className="pos-label">{playPos}</span>}</div>); })}</div>
        </div>
